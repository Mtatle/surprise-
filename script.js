...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const ITEMS = [
    ...
    { label:'Mohammed', type:'message', message:'Mohammed' }
  ];
  const COLORS = [0xff6666,0x66ffcc,0x66ccff,0x8866ff,0xffcc66];

  // No textures for decorative; we'll use random pastel HSL
  ...
  function init() {
    ...
    makeDecorativePlanets(20); // 20 decorative planets
    makeMeteors(50);
  }
  ...
  function makeStars() {
    ...
    for(let i=0;i<N;i++){const th=Math.random()*Math.PI*2, ph=Math.acos(2*Math.random()-1), r=1000;
  ...
  function makeInteractivePlanets() {
    ITEMS.forEach((item,i)=>{
      ...
      mesh.position.set(radius,height,0); pivot.add(mesh);
      const label=makeLabel(item.label); label.position.set(radius,height+5,0); pivot.add(label);
  ...
  function makeDecorativePlanets(count) {
    for(let i=0;i<count;i++){
      const pivot=new THREE.Object3D(); pivot.visible=false; scene.add(pivot);
      const th=Math.random()*Math.PI*2, ph=Math.acos(2*Math.random()-1), r=50+Math.random()*500;
      const x=r*Math.sin(ph)*Math.cos(th), y=-30+Math.random()*60, z=-20-r*Math.cos(ph);
      // random pastel color
      const h=Math.random(), s=0.5+Math.random()*0.3, l=0.5+Math.random()*0.3;
      const color=new THREE.Color().setHSL(h,s,l);
      const mat=new THREE.MeshStandardMaterial({ color, emissive: color.clone().multiplyScalar(0.2) });
      const size = (i<3 ? 70+Math.random()*30 : 2+Math.random()*8); // 3 huge ones
  ...
  function onClick(e) {
    ...
    if(hit){
      const idx=meshes.indexOf(hit.object), it=interactive[idx].item;
      if(it.type==='video'){
        bgMusic.pause();
  ...
  function spawnParticles(pos) {
    ...
    scene.add(points);
    particles.push({ points, velocities });
  }
  ...
  function animate() {
    ...
    interactive.forEach((o,i)=>{
      o.group.rotation.y+=0.0005 + i*0.0001;
      o.mesh.rotation.y+=0.002 + i*0.0005;
    });
    decorative.forEach((o,i)=>{
      o.group.rotation.y+=0.0002 + i*0.00005;
      o.mesh.rotation.y+=0.001 + i*0.0002;
    });
    meteors.forEach(m=>{
      m.mesh.visible=true;
    ...
    for(let i=particles.length-1;i>=0;i--){
      ...
      p.velocities.forEach((v,j)=>{
        attr.array[j*3]+=v.x;
        attr.array[j*3+1]+=v.y;
        attr.array[j*3+2]+=v.z;
      });
      attr.needsUpdate=true;
      p.points.material.opacity-=0.02;
      if(p.points.material.opacity<=0){
